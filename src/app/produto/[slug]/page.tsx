    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Confira ${product?.name || 'este produto'} na Sarai Modas!`,
                    ...
                    {sizes.map((sizeData: any) => {
                      ...
                      return (
                        <motion.button
                          ...
                          className={`py-3 px-2 rounded-xl border-2 font-bold transition-all duration-300 relative ${
                            isOutOfStock
                              ? 'cursor-not-allowed size-out-of-stock bg-dark-800'
                              : isSelected
                              ? 'border-primary-500 bg-primary-500 text-white shadow-lg shadow-primary-500/25 size-available'
                              : 'border-dark-700 text-gray-300 hover:border-primary-400 hover:bg-dark-800 size-available'
                          }`}
                        >
                ...
                {selectedSize && (
                  <div className="mt-4 p-3 bg-dark-800 rounded-lg border border-dark-700">
                    <p className="text-sm text-gray-300">
                      Tamanho selecionado: <span className="font-semibold text-white">{selectedSize}</span>
                    </p>
                  </div>
                )}
              </div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <button
                  onClick={() => {
                    window.open(`https://wa.me/?text=Confira ${product?.name || 'este produto'} na Sarai Modas! ${window.location.href}`, '_blank');
                    setShowShareModal(false);
                  }}
