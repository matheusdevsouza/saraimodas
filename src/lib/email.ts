...
export async function sendVerificationEmail(data: VerificationEmailData): Promise<void> {
  ...
  const mailOptions = {
    ...
    to: data.email,
    subject: "Verifique sua conta - Sarai Modas",
    html: `
      <!DOCTYPE html>
      <html>
      <head>
...
export async function sendPasswordResetEmail(data: PasswordResetEmailData): Promise<void> {
  ...
  const mailOptions = {
    from: `"Sarai Modas" <${process.env.SMTP_FROM || process.env.EMAIL_FROM}>`,
    to: data.email,
    subject: "Redefinir Senha - Sarai Modas",
    html: `
      <!DOCTYPE html>
      <html>
      <head>
...
export async function sendTrackingEmail(data: TrackingEmailData): Promise<void> {
  ...
  const mailOptions = {
    from: `"Sarai Modas" <${process.env.SMTP_FROM || process.env.EMAIL_FROM}>`,
    to: data.email,
    subject: `Rastreamento do Pedido ${data.orderNumber} - Sarai Modas`,
    html: `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Rastreamento do Pedido - Sarai Modas</title>
        <style>
          body {
...
export async function sendPaymentConfirmationEmail(data: PaymentConfirmationEmailData): Promise<void> {
  ...
  const mailOptions = {
    from: `"Sarai Modas" <${process.env.SMTP_FROM || process.env.EMAIL_FROM}>`,
    to: data.email,
    subject: `Pagamento Aprovado - Pedido ${data.orderNumber} - Sarai Modas`,
    html: `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Pagamento Aprovado - Sarai Modas</title>
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; }
          .header { background: linear-gradient(135deg, #e63946 0%, #b71c1c 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
...
export async function sendOrderShippedEmail(data: OrderShippedEmailData): Promise<void> {
  ...
  const mailOptions = {
    from: `"Sarai Modas" <${process.env.SMTP_FROM || process.env.EMAIL_FROM}>`,
    to: data.email,
    subject: `Seu Pedido Foi Enviado! - ${data.orderNumber} - Sarai Modas`,
    html: `
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>Pedido Enviado - Sarai Modas</title>
        <style>
          body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; }
          .header { background: linear-gradient(135deg, #e63946 0%, #b71c1c 100%); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
